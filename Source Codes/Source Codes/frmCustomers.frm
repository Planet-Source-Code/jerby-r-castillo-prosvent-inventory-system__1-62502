...
If Trim(txtname.text) <> "" Then
     ...
     Call CustSave
Else
    MsgBox "Please Input atleast Customer name", vbInformation
End If
...
Set Rs1 = DCON.Execute("Select * from customer where company='" & txtname.text & "'")
...
CustID = CIns.AUTONUM(CDb.OpenDb, "Customer", "CustomerID", "CUST")

CDb.TableName = "Customer"
...
Else
    MsgBox "The Customer Name was Exist", vbInformation, "Customer"
...
CDb.TableName = "Customer"
CDb.ClauseStatement = "Where customerID='" & MODIFYID & "'"
...
If ADDING = False Then
    Call GetNewConnection2
    Set Rs1 = New Recordset
    Set Rs1 = DCON.Execute("Select * from customer where customerID='" & MODIFYID & "'")

    If Rs1.RecordCount <> 0 Then
        txtname.text = Rs1!company & " "
